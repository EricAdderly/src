...
// TranslationRepo -.
type ReceivingTransactionRepo struct {
	*postgres.Postgres
}

func New(pg *postgres.Postgres) *ReceivingTransactionRepo {
	return &ReceivingTransactionRepo{pg}
}

func (r *ReceivingTransactionRepo) GetSession(ctx context.Context, session string) error {
	return nil
}
...
// 		ToSql()
// 	if err != nil {
// 		return nil, fmt.Errorf("Authrisation - CheckSessionsData - a.Builder: %w", err)
// 	}

// 	rows, err := a.Pool.Query(ctx, sql, session.AccessToken)
// 	if err != nil {
// 		return nil, fmt.Errorf("Authrisation - CheckSessionsData - a.Pool.Query: %w", err)
// 	}
// 	defer rows.Close()
...
// 		ToSql()
// 	if err != nil {
// 		return nil, fmt.Errorf("Authrisation - GetUserFromDb - a.Builder: %w", err)
// 	}

// 	rows, err := a.Pool.Query(ctx, sql, rq.Login)
// 	if err != nil {
// 		return nil, fmt.Errorf("Authrisation - GetUserFromDb - a.Pool.Query: %w", err)
// 	}
// 	defer rows.Close()
...
// 		if err != nil {
// 			return nil, fmt.Errorf("Authrisation - GetUserFromDb - rows.Scan: %w", err)
// 		}
